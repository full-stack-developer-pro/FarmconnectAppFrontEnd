...
const Agronomist = ({ navigation }) => {
  ...
  const getIndex = (key) => (event) => {
    ...
    return navigation.navigate("Myquastion", { key: `${data}` });
  };
      ...
      <View style={styles.containerMain}>
        <View style={{ height: height - height / 12 }}>
          ...
          <View style={styles.textAriaMain}>
            <Image
              style={styles.textAreaImg}
              source={require("../assets/Agronomist_page_logo.png")}
            />
            <View style={styles.textArea}>
              <Text style={styles.textAriaHeadind}>
                {i18n.t("Ask_your_question")}
              </Text>
              <TextInput
                style={styles.input}
                value={question}
                onChangeText={TypeQuestion}
                placeholder={`${i18n.t("Type_here")}....`}
              />
            </View>
          </View>
          <ImagePickerExample
            StartCamera={() => navigation.navigate("CameraStart")}
          />
          ...
          <View style={styles.MyquestionMainContainer}>
            <AskFilter
              my={i18n.t("My_questions")}
              other={i18n.t("Other_questions")}
            />
            ...
            <ScrollView>
              {Data.map((item, ind) => {
                return (
                  <TouchableOpacity
                    key={ind}
                    style={styles.AnnouncementMain}
                    onPress={getIndex(ind)}
                  >
                    <Image
                      style={styles.AnnouncementImg}
                      source={require("../assets/askimage.png")}
                    />
                    <View style={styles.Announcement}>
